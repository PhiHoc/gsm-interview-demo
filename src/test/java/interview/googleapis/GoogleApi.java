...
public class GoogleApi {
    ...
    private static final File DATA_STORE_DIR = new File(System.getProperty("user.home"), ".credentials/sheets.googleapis.com");
    ...
    public static Drive getDriveService(String credentialsPath) {
        try (InputStream in = GoogleApi.class.getResourceAsStream(credentialsPath)) {
    ...
    /**
     * Creates and returns an authorized Sheets API client service.
     * Determines the authorization type (OAuth2 or Service Account) based on the JSON structure.
     *
     * @param credentialsPath The path to the credentials JSON file.
     * @return Authorized Sheets API client.
     * @throws RuntimeException if there is an issue reading the credentials.
     */
    public static Sheets getSheetsService(String credentialsPath) {
        try (InputStream in = GoogleApi.class.getResourceAsStream(credentialsPath)) {
            byte[] credentialsData = in.readAllBytes();
            return isServiceAccount(credentialsData)
                    ? getServiceSheetAccount(credentialsData)
                    : getOAuthSheetAccount(credentialsData);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    /**
     * Creates an authorized Sheets API client for a service account.
     *
     * @param credentialsData Byte array containing the service account credentials JSON data.
     * @return Sheets API client for a service account.
     * @throws IOException if there is an issue reading the credentials.
     */
    private static Sheets getServiceSheetAccount(byte[] credentialsData) throws IOException {
    ...
    /**
     * Creates an authorized Sheets API client using OAuth2.
     *
     * @param credentialsData Byte array containing the OAuth2 credentials JSON data.
     * @return Sheets API client for OAuth2 authorization.
     * @throws IOException if there is an issue reading the credentials.
     */
    private static Sheets getOAuthSheetAccount(byte[] credentialsData) throws IOException {
